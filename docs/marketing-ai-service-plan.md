...
`Hey Mark`는 창의적인 마케팅 조언을 빠르게 제공하는 AI 서비스다. 사용자는 업종, 제품, 고객, 예산, 채널, 현재 고민을 입력하고 다음 결과를 받는다.
...
가장 가볍고 빠른 제공 방식은 웹 기반 채팅형 서비스다.

사용 흐름:

1. 사용자가 업종, 제품, 고객, 목표, 예산, 채널을 입력한다.
2. AI가 부족한 정보를 2-4개만 질문한다.
3. 지식베이스에서 관련 마케팅 원칙, 사례, 프레임워크를 검색한다.
4. 사용자의 상황에 맞춰 실행 가능한 제안을 만든다.
5. 답변 하단에 참고 지식과 근거를 표시한다.
...
- `goal`: 인지도, 전환, 재구매, 리드 확보 등
- `budget_level`: 없음, 소액, 중간, 큼
- `channels`: Instagram, YouTube, Blog, Search Ads, Offline 등
